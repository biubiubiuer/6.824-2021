	...
	for {
		...
		case MAP:
			if len(reply.Filenames) < 1 {
				log.Fatalf("don't habe filename")
			}
...
func DoMAP(filename string, task_to int, nReduce int, mapf func(string, string) []KeyValue) {
	...
	// atomically rename
	for i := 0; i < nReduce; i++ {
		filename_tmp := fmt.Sprintf("mr-%d-%d", task_to, i)
...
// rpc interface
func CallGetTast(args *GetTaskArgs, reply *GetTaskReply) bool {
